 ...
 */
class DimbalPollManager_DPM_FREE{
    ...
    */
    public static function wpActionPluginsLoaded(){
        Dimbal_DPM_FREE::checkForUpgrade(Dimbal_DPM_FREE::buildDatabaseVersionString(), self::CURRENT_VERSION, array(Dimbal_DPM_FREE::buildAppClassName('DimbalPollManager'),'installDatabase'));
    }
    ...
    public static function wpActionWidgetsInit(){
        register_widget( Dimbal_DPM_FREE::buildAppClassName('DimbalPollWidget') );
    }
    ...
    */
    ...
    public static function wpActionAdminMenu(){
        ...
        add_menu_page( 'Dimbal Poll Manager', DIMBAL_CONST_DPM_FREE_PLUGIN_TITLE_SHORT, 'manage_options', Dimbal_DPM_FREE::buildPageSlug(self::PAGE_HOME), array(Dimbal_DPM_FREE::buildAppClassName('Dimbal'),'renderPage') );

        //add_submenu_page( $parent_slug, $page_title, $menu_title, $capability, $menu_slug, $function);
        add_submenu_page( Dimbal_DPM_FREE::buildPageSlug(self::PAGE_HOME), 'Poll Manager', 'Poll Manager', 'manage_options', Dimbal_DPM_FREE::buildPageSlug(self::PAGE_POLLS), array(Dimbal_DPM_FREE::buildAppClassName('Dimbal'),'renderPage'));
        add_submenu_page( Dimbal_DPM_FREE::buildPageSlug(self::PAGE_HOME), 'Zone Manager', 'Zone Manager', 'manage_options', Dimbal_DPM_FREE::buildPageSlug(self::PAGE_ZONES), array(Dimbal_DPM_FREE::buildAppClassName('Dimbal'),'renderPage'));
        add_submenu_page( Dimbal_DPM_FREE::buildPageSlug(self::PAGE_HOME), 'Reports', 'Reports', 'manage_options', Dimbal_DPM_FREE::buildPageSlug(self::PAGE_REPORTS), array(Dimbal_DPM_FREE::buildAppClassName('Dimbal'),'renderPage'));
        add_submenu_page( Dimbal_DPM_FREE::buildPageSlug(self::PAGE_HOME), 'Settings & Tools', 'Settings & Tools', 'manage_options', Dimbal_DPM_FREE::buildPageSlug(self::PAGE_SETTINGS), array(Dimbal_DPM_FREE::buildAppClassName('Dimbal'),'renderPage'));
        add_submenu_page( Dimbal_DPM_FREE::buildPageSlug(self::PAGE_HOME), 'Help & Support', 'Help & Support', 'manage_options', Dimbal_DPM_FREE::buildPageSlug(self::PAGE_SUPPORT), array(Dimbal_DPM_FREE::buildAppClassName('Dimbal'),'renderPage'));

        add_submenu_page( 'fake-slug-does-not-exist', 'Preview', 'Preview', 'manage_options', Dimbal_DPM_FREE::buildPageSlug(self::PAGE_PREVIEW), array(Dimbal_DPM_FREE::buildAppClassName('Dimbal'),'renderPage'));
    }
    ...
    public static function installDatabase(){
        ...
        $question_table_name = DimbalPollQuestion_DPM_FREE::getTableName();
        $response_table_name = DimbalPollResponse_DPM_FREE::getTableName();
        $sql = "
        ...
        DimbalSetting_DPM_FREE::installDatabase();

        // Setup the Zones Table
        DimbalZoneManager_DPM_FREE::installDatabase();

        // Add an option to store the version number
        add_option( Dimbal_DPM_FREE::buildDatabaseVersionString(), self::CURRENT_VERSION );
    }
    ...
    */
    ...
    public static function buildSampleData(){
        ...
        $zone1 = new DimbalZone_DPM_FREE();
        $zone1->typeId = DimbalZone_DPM_FREE::TYPE_DPM;
        $zone1->text = "Music";
        $zone1->status = DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE;
        $zone1->save();

        $zone2 = new DimbalZone_DPM_FREE();
        $zone2->typeId = DimbalZone_DPM_FREE::TYPE_DPM;
        $zone2->text = "Movies";
        $zone2->status = DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE;
        $zone2->save();
        ...
        $poll1 = new DimbalPollQuestion_DPM_FREE();
        $poll1->text = "Which music genre is the best?";
        $poll1->hitCount = 123;
        $poll1->lastHitDate = rand($timeStart, $timeEnd);
        $poll1->multipleResponses = true;
        $poll1->status = DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE;
        $poll1->save();
        $poll1->saveAnswerChoice(0, "Classic Rock", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll1->saveAnswerChoice(1, "Popular", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll1->saveAnswerChoice(2, "Country", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll1->saveAnswerChoice(3, "Heavy Metal", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll1->saveAnswerChoice(4, "Rap", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll1->saveAnswerChoice(5, "R&B", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        self::sampleDataHelperCreateResponses($poll1, 5, 10);

        $poll2 = new DimbalPollQuestion_DPM_FREE();
        $poll2->text = "Which performer puts on a better show?";
        $poll2->hitCount = 254;
        $poll2->lastHitDate = rand($timeStart, $timeEnd);
        $poll2->multipleResponses = true;
        $poll2->status = DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE;
        $poll2->save();
        $poll2->saveAnswerChoice(0, "Beyonce", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll2->saveAnswerChoice(1, "Kid Rock", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll2->saveAnswerChoice(2, "U2", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll2->saveAnswerChoice(3, "Eminem", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll2->saveAnswerChoice(4, "Snoop Dogg", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll2->saveAnswerChoice(5, "Keith McGraw", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        self::sampleDataHelperCreateResponses($poll2, 5, 16);

        $poll3 = new DimbalPollQuestion_DPM_FREE();
        ...
        $poll3->status = DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE;
        $poll3->save();
        $poll3->saveAnswerChoice(0, "Batman", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll3->saveAnswerChoice(1, "Lord of the Rings", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll3->saveAnswerChoice(2, "Jason Bourne", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll3->saveAnswerChoice(3, "X-Men", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll3->saveAnswerChoice(4, "Fast and Furious", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        self::sampleDataHelperCreateResponses($poll3, 4, 8);

        $poll4 = new DimbalPollQuestion_DPM_FREE();
        $poll4->text = "Which movie genre is the best?";
        $poll4->hitCount = 123;
        $poll4->lastHitDate = rand($timeStart, $timeEnd);
        $poll4->multipleResponses = true;
        $poll4->status = DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE;
        $poll4->save();
        $poll4->saveAnswerChoice(0, "Action", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll4->saveAnswerChoice(1, "Comedy", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll4->saveAnswerChoice(2, "Horror", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll4->saveAnswerChoice(3, "Drama", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
        $poll4->saveAnswerChoice(4, "Musical", DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE);
    ...
    public static function enqueuePublicResources(){
        wp_enqueue_style( DIMBAL_CONST_DPM_FREE_SLUG.'-main-css', DIMBAL_CONST_DPM_FREE_URL.'/css/dimbal-main.css' );
        wp_enqueue_script( DIMBAL_CONST_DPM_FREE_SLUG.'-main-js', DIMBAL_CONST_DPM_FREE_URL.'/js/dimbal-dpm.js', array( 'jquery' ) );

        // Don't forget to localize the data objects we want
        $dimbalVars = array(
            'slug' => DIMBAL_CONST_DPM_FREE_SLUG,
            'url' => DIMBAL_CONST_DPM_FREE_URL,
            'ajax_url' => admin_url('admin-ajax.php'),
        );
        wp_localize_script( DIMBAL_CONST_DPM_FREE_SLUG.'-main-js', 'dimbal_dpm_vars', $dimbalVars );
    }

    public static function enqueueAdminResources(){
        wp_enqueue_style( DIMBAL_CONST_DPM_FREE_SLUG.'-admin-css', DIMBAL_CONST_DPM_FREE_URL.'/css/dimbal-admin.css' );

        /*
        wp_enqueue_script( DIMBAL_CONST_DPM_FREE_SLUG.'-admin-js', DIMBAL_CONST_DPM_FREE_URL.'/js/dimbal-admin.js', array( 'jquery' ) );
        ...
        wp_localize_script( DIMBAL_CONST_DPM_FREE_SLUG.'-admin-js', 'dimbal_admin_vars', $dimbalVars );
        */
    }
    ...
    public static function renderPage(){
        Dimbal_DPM_FREE::renderPage();
    }

    public static function buildSettingsEditorOptions(){

        $object = DimbalSetting_DPM_FREE::getSettingsObject();
    ...
    public static function ajaxGetPublicHandlerMappings(){
        $handlers = array(
            'DIMBAL_CONST_DPM_FREE_SLUG-display-poll'=>array(Dimbal_DPM_FREE::buildAppClassName('DimbalPollManager'),'ajaxDisplayPoll'),
            'DIMBAL_CONST_DPM_FREE_SLUG-submit-poll'=>array(Dimbal_DPM_FREE::buildAppClassName('DimbalPollManager'),'ajaxSubmitPoll'),
            'DIMBAL_CONST_DPM_FREE_SLUG-view-results'=>array(Dimbal_DPM_FREE::buildAppClassName('DimbalPollManager'),'ajaxViewResults')
        );
        return $handlers;
    }
    ...
    public static function ajaxGetAdminHandlerMappings(){
        $handlers = array(
            'DIMBAL_CONST_DPM_FREE_SLUG-admin-poll-analysis'=>array(Dimbal_DPM_FREE::buildAppClassName('DimbalPollManager'),'ajaxViewAdminPollAnalysis'),
            'DIMBAL_CONST_DPM_FREE_SLUG-admin-zone-analysis'=>array(Dimbal_DPM_FREE::buildAppClassName('DimbalPollManager'),'ajaxViewAdminZoneAnalysis'),
            'DIMBAL_CONST_DPM_FREE_SLUG-admin-report'=>array(Dimbal_DPM_FREE::buildAppClassName('DimbalPollManager'),'ajaxViewAdminReport')
        );
        return $handlers;
    }

    public static function ajaxProcessHandlerWrapper(){
        Dimbal_DPM_FREE::ajaxProcessHandler(self::ajaxGetAllHandlerMappings());
    }
    ...
    public static function ajaxDisplayPoll(){
        ...
        // Prepare the poll
        $poll = false;
        $dpmEId = Dimbal_DPM_FREE::getRequestVarIfExists('dpmEId');
        ...
        //$zoneDisplayAll = Dimbal_DPM_FREE::getRequestVarIfExists('zoneDisplayAll');
        ...
        if(!empty($pollId)){
            ...
            if(!empty($zone)){
                ...
                $polls = DimbalPollQuestion_DPM_FREE::getAllByZoneId($zoneId);
                if(!empty($polls)){
                    shuffle($polls);
                    $validated = false;
                    while($validated==false && !empty($polls)){
                        $poll = array_shift($polls);
                        if($poll->status != DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE){
    ...
    public static function ajaxHelperDisplaySinglePoll($poll, $dpmEId, $createWrapper=false){
        ...
        //Dimbal_DPM_FREE::logMessage("Inside ".__FUNCTION__);
        ...
        if($poll){
            if($poll->status != DimbalStandardObjectRecord_DPM_FREE::STATUS_ACTIVE){
                ...
                $html .= '<input type="button" id="dpmSubmit_'.$dpmEId.'" onClick="javascript:dimbalPoll_DPM_FREE.submitPoll(\''.$dpmEId.'\');" value="Submit" />';
                $html .= '</form>';

                if($poll->viewEarlyResults || $poll->hasAlreadyResponded()){
                    $html .= '<p><a href="javascript:dimbalPoll_DPM_FREE.viewEarlyResults(\''.$dpmEId.'\')">View Results</a></p>';
                }
    ...
    public static function ajaxSubmitPoll(){
        ...
        $pollId = Dimbal_DPM_FREE::getRequestVarIfExists('pollId');
        $poll = DimbalPollQuestion_DPM_FREE::get($pollId);
        $pollChoice = Dimbal_DPM_FREE::getRequestVarIfExists('pollChoice');

        $html = '';
        $html .= '<div style="max-width:250px;">';
        if(empty($poll)){
            $html .= '<p><img src="'.DIMBAL_CONST_DPM_FREE_URL_IMAGES.'/error.png" style="vertical-align:middle" /> Error validating Poll.  Please try again.</p>';
        }elseif(empty($pollChoice)){
            $html .= '<p><img src="'.DIMBAL_CONST_DPM_FREE_URL_IMAGES.'/error.png" style="vertical-align:middle" /> Error validating Poll Choice.  Please try again.</p>';
        }elseif($poll->recordResponse($pollChoice)){
            if($poll->viewResults){
                $html .= self::ajaxHelperDisplayPollResults($poll, $dpmEId);
            }else{
                $html .= '<p><img src="'.DIMBAL_CONST_DPM_FREE_URL_IMAGES.'/tick.png" style="vertical-align:middle" /> Response submitted.</p>';
            }
        }else{
            $html .= '<p><img src="'.DIMBAL_CONST_DPM_FREE_URL_IMAGES.'/error.png" style="vertical-align:middle" /> There was an error submitting your response.  Please try again.</p>';
        }
        $html .= '</div>';
        $response['html']=$html;
    ...
    public static function ajaxHelperDisplayPollResults($poll, $dpmEId, $createWrapper=false){
        ...
        if($poll->viewResults){
            $responses = DimbalPollResponse_DPM_FREE::getAllByPollId($poll->id);
    ...
    public static function ajaxViewResults(){
        ...
        $html .= '<div style="max-width:250px;">';
        $html .= self::ajaxHelperDisplayPollResults($poll, $dpmEId);
        $html .= '<div><a href="javascript:dimbalPoll_DPM_FREE.displayPoll('.$pollId.',\''.$dpmEId.'\');">Back to Poll Voting</a></div>
		';
        $html .= '</div>';
        $response['html']=$html;
    ...
    public static function ajaxViewAdminZoneAnalysis(){
        ...
        $zoneId = Dimbal_DPM_FREE::getRequestVarIfExists('zoneId');
        if(!empty($zoneId)){
            $zone = DimbalPollQuestion_DPM_FREE::get($zoneId);
            if(!empty($zone)){

                $polls = DimbalPollQuestion_DPM_FREE::getAllByZoneId($zone->id);
    ...
    public static function ajaxViewAdminReport(){
        ...
        $reportType = Dimbal_DPM_FREE::getRequestVarIfExists('reportType');
        switch($reportType){
            case '1':
    ...
    public static function shortcodeHandler($atts){
        ...
        if(!empty($poll_id)){
            $poll = DimbalPollQuestion_DPM_FREE::get($poll_id);
            if(!empty($poll)){
